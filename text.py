...
class TextGroup(object):
    ...
    def addText(self, text, colour, x, y, size, time=None, id=None):
        self.nextid += 1
        self.alltext[self.nextid] = Text(text, colour, x, y, size, time, id)
        return self.nextid
    ...
    def update(self, dt):
        for key in list(self.alltext.keys()):
            self.alltext[key].update(dt)
            if self.alltext[key].destroy:
                self.removeText(key)
    ...
    def render(self, screen):
        for key in self.alltext.keys():
            self.alltext[key].render(screen)
